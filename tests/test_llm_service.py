...
async def test_process_without_context(llm_service):
    with patch.object(
        llm_service, "_invoke_with_retry", new_callable=AsyncMock, return_value="응답 결과"
    ):
...
async def test_process_with_context(llm_service):
    with patch.object(
        llm_service, "_invoke_with_retry", new_callable=AsyncMock, return_value="컨텍스트 응답"
    ):
...
async def test_summarize(llm_service):
    with patch.object(
        llm_service, "_invoke_with_retry", new_callable=AsyncMock, return_value="요약 결과"
    ):
...
async def test_classify_with_categories(llm_service):
    with patch.object(
        llm_service, "_invoke_with_retry", new_callable=AsyncMock, return_value="긍정"
    ):
...
async def test_classify_without_categories(llm_service):
    with patch.object(
        llm_service, "_invoke_with_retry", new_callable=AsyncMock, return_value="기술"
    ):
...
async def test_generate_workflow_success(llm_service):
    ...
    with patch.object(
        llm_service, "_invoke_with_retry", new_callable=AsyncMock, return_value=mock_result
    ):
...
async def test_retry_on_rate_limit(llm_service):
    chain = AsyncMock()
    chain.ainvoke = AsyncMock(side_effect=[Exception("rate limit exceeded"), "성공"])
...
async def test_rate_limit_max_one_retry(llm_service):
    chain = AsyncMock()
    chain.ainvoke = AsyncMock(side_effect=[Exception("rate limit"), Exception("rate limit")])
